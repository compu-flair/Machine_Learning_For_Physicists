...
class Segment3(Scene):
    def construct(self):
        ...
        self.play(prob_eq[0][11:13].animate.set_opacity(1),prob_eq[0][:3].animate.set_opacity(0))
        self.play(
                LaggedStart(
        ...
        self.play(Write(expansion2),Write(expansion3))
        ...
        rect16 = SurroundingRectangle(Group(eq,replacement_part1,replacement_part2,subsection1,replacement_part4,prob_eq)).scale([0.85,0.3,1]).shift(RIGHT*0.4+UP*0.2)
        self.play(Create(rect16))
        self.play(Uncreate(rect16))
        ...
        table_pos = table.get_center()
        self.play(Create(table))
        self.play(problem2.animate.next_to(problem1, RIGHT*3 , aligned_edge=UP), FadeOut(table))
        self.wait(16)
        columns = [
            "Index",               
            "Label",               
        ...
        table.scale(0.35)
        table.next_to(problem1, DOWN*14, aligned_edge=LEFT)
        self.play(ReplacementTransform(SurroundingRectangle(dataset1),table), FadeOut(problem2))
        self.wait(4)
        ...
        for dots in dot_groups:
            self.play(LaggedStart(*[FadeIn(dot) for dot in dots], lag_ratio=0.05), run_time=0.1)
        ...
        self.wait(1.5)
        self.play(Indicate(Group(mean_labels[0],mean_labels[1],mean_labels[2],mean_labels[3],mean_labels[4],mean_labels[5]),scale_factor=1.5))
        ...
        self.wait(7)
        self.play(FadeOut(problem1,dataset1, graph, rect3), table.animate.shift(RIGHT*4), system_group.animate.shift(RIGHT*2))
        equation1 = MathTex(
        ...
        self.wait(0.5)
        self.play(Indicate(Group(x1, x2, x3, x4, xn)))
        ...
        self.wait(10)
        self.play(table.animate.shift(LEFT*5+UP))
        self.wait(4)
        new_label = MathTex("y", color=YELLOW).scale(0.6).move_to(table.get_entries((1,6)).get_center())
        self.play(ReplacementTransform(table.get_entries((1,6)),new_label))
        
        rect = SurroundingRectangle(table.get_columns()[:-1], color=YELLOW).scale(1.1).shift(LEFT*0.25)
        self.play(Create(rect))
        X_label = MathTex("X", color=YELLOW).scale(0.7).next_to(table,UP*1.5)
        ...
        """,
        color=ORANGE
        ).scale(0.7).next_to(reservoir, DOWN*5, aligned_edge=RIGHT).shift(RIGHT*3)
        new_label_1 = MathTex(r"\vec{y}", color=YELLOW).scale(0.6).move_to(table.get_entries((1,6)).get_center())
        X_label_1 = MathTex(r"\vec{X}", color=YELLOW).scale(0.7).next_to(table,UP*1.5)
        self.play(FadeIn(equation6, shift=LEFT*3.5), ReplacementTransform(new_label,new_label_1), ReplacementTransform(X_label,X_label_1))
        self.wait(1)
        ...
        rect17 = SurroundingRectangle(equation7[0][2])
        self.play(Group(table,rect,new_label_1,X_label_1).animate.shift(DOWN*1.5+RIGHT*5), equation6.animate.scale(0.8).shift(UP*2+RIGHT),FadeIn(equation7))
        self.play(Create(rect17))
        self.wait()
        rect18 = SurroundingRectangle(equation7[0][4])
        self.play(Create(rect18))
        self.wait(2)
        self.play(Uncreate(rect17),Uncreate(rect18))
        self.wait()
        self.play(Indicate(equation7))
        self.wait(3)
        ...
        arrow1 = Arrow(start=equation9.get_top(),
                       ...
                       stroke_width=2)
        label1 = Text("Maximum Entropy Principle", font="Orbitron", color=YELLOW).scale(0.4).next_to(arrow1, UP)

        # Arrow 2 (right side)
        arrow2 = Arrow(start=equation9.get_right() ,
                       end=equation9.get_right() + RIGHT,
                       stroke_width=2)
        label2 = Text("Canonical Ensemble ", font="Orbitron", color=YELLOW).scale(0.4).next_to(arrow2.get_end(), RIGHT)

        # Arrow 3 (lower left)
        arrow3 = Arrow(start=equation9.get_bottom() ,
                       end=equation9.get_bottom()+ DOWN,
                       stroke_width=2)
        label3 = Text("Partition Function", font="Orbitron", color=YELLOW).scale(0.4).next_to(arrow3.get_end(), DOWN)
        ...
        rect12 = SurroundingRectangle(Group(table, X_label_1,rect),color=PINK)
        ...
        self.wait(4)
        self.play(FadeOut(Group(*[obj for obj in self.mobjects if obj not in [entropy]])))
        formula = MathTex(r"S = - \sum_i p_i \ln p_i", color=ORANGE).scale(0.5).next_to(entropy,DOWN)
        self.play(Create(formula))
        self.wait(7)
        self.play(ShowPassingFlash(Line(LEFT*0.8, RIGHT*0.8).next_to(formula, DOWN*0.2)))
        formula1 = MathTex(r"S_{max} = \text{equilibrium state}", color=ORANGE).scale(0.5).next_to(formula,DOWN, aligned_edge=LEFT)
        self.play(Create(formula1))
        self.wait()
        self.wait()
        expansion_box = RoundedRectangle(height=1, width=5).set_fill(color=TEAL, opacity=0.2).shift(UP*0.5)
        self.play(Create(expansion_box))
        eq1 = MathTex(r"P(x)", color=YELLOW).scale(0.5).next_to(formula, DOWN*4+LEFT*3)
        eq2 = MathTex(
        ...
        self.wait()
        self.play(Group(expansion_box,eq1,eq2,eq3,eq4).animate.shift(LEFT*4))
        self.wait(2)
        equation9 = MathTex(r"\hat{y} = \beta_0 + \beta \cdot X",color=PURE_GREEN).scale(0.7).shift(RIGHT)
        self.play(FadeIn(equation9))
        self.wait(2)
        equation10 = MathTex(r"\beta_0 = \bar{y}",color=PURE_GREEN).scale(0.4).next_to(equation9, DOWN*2, aligned_edge=LEFT)
...
#             # Transform snapshot into table row
#             self.play(ReplacementTransform(snapshot_group, row), run_time=0.7)  
#         self.play(Create(table.vertical_lines), Create(table.horizontal_lines))
#         self.wait()
